...
These changes need a Logfire release that sets SLO channels per tier alert and returns them on read. Do not upgrade the provider before your Logfire instance has that release.

- `logfire_alert`: `channel_ids` is replaced by `channel_assignments`, a set of `{ channel_id, schedule_id }` objects. `schedule_id` is optional. It limits the channel to the windows of a `logfire_schedule`. The provider sends and reads the value 1:1 with the alert API.
